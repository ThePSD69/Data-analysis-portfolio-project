...
-- Creating a result with the necesary data for our analysis

SELECT DISTINCT year, genre, AVG(score)  OVER(PARTITION BY year, genre) 'Average Score by Genre and Year'
FROM PortfolioProject..movies
ORDER BY 2

SELECT DISTINCT year, genre, AVG(budget_rec_perc) OVER(PARTITION BY year, genre)  AS 'Average Recovery % by Gender and Year'
FROM PortfolioProject..movies
ORDER BY 3
